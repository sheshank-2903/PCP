    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db.collection("Students").document(user.getEmail()).addSnapshotListener(new EventListener<DocumentSnapshot>() {
            ...
            public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                if (value.exists()) {
                    if (value.get("profilePicture")!=null && value.get("profilePicture").toString().length() > 0) {
        ...
        aboutUsText.setText("Poornima Connecting People is an interactive where students and alumni of a college can interact with each other and " +
